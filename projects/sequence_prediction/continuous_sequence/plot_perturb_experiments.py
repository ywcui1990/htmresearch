...
truthLSTMonlinePerturb = expResultPerturb.truth
...
filePath = './prediction/' + dataSet + '_elm_pred.csv'
predDataELM = pd.read_csv(filePath, header=0, skiprows=[1, 2],
                          names=['step', 'value', 'prediction5'])
elmPredPerturbTruth = np.roll(predDataELM['value'], -5)
elmPredPerturb = np.array(predDataELM['prediction5'])
...
mapeELMPerturb = np.abs(elmPredPerturb - elmPredPerturbTruth)


data = readExperiment('results/nyc_taxi_experiment_continuous_perturb/learning_window'+str(1001.0)+'/0.log')
...
expResultPerturbOnline = ExperimentResult(
  'results/nyc_taxi_experiment_continuous_perturb_online/learning_window'+str(100.0)+'/')
...
plotAccuracy((mapeTMPerturb, xaxisDatetime), tmTruthPerturb,
             window=window, errorType='mape', label='HTM')

plotAccuracy((mapeLSTM3000Perturb, xaxisDatetime), expResultPerturb3000.truth,
             window=window, errorType='mape', label='LSTM3000')

plotAccuracy((mapeLSTM6000Perturb, xaxisDatetime), expResultPerturb6000.truth,
             window=window, errorType='mape', label='LSTM6000', train=expResultPerturb6000.train)
#
# plotAccuracy((mapeELMPerturb, xaxisDatetime), elmPredPerturbTruth,
#              window=window, errorType='mape', label='ELM')
#
# plotAccuracy((mapeESNPerturb, xaxisDatetime), esnPredPerturbTruth,
#              window=window, errorType='mape', label='ESN')
#
# plotAccuracy((mapeLSTM1000Perturb, xaxisDatetime), expResultPerturb1000.truth,
#              window=window, errorType='mape', label='LSTM1000', train=expResultPerturb1000.train)
#
# plotAccuracy((mapeLSTMOnlinePerturb, xaxisDatetime), truthLSTMonlinePerturb,
...
plotAccuracy((negLLLSTM6000Perturb, xaxisDatetime), truthLSTM6000Perturb,
             window=window, errorType='negLL', label='LSTM6000')
plotAccuracy((negLLLSTMonlinePerturb, xaxisDatetime), truthLSTM6000Perturb,
             window=window, errorType='negLL', label='LSTM-online')

plotAccuracy((negLLTMPerturb, xaxisDatetime), tmTruthPerturb, window=window,
             errorType='negLL', label='TM')
plt.axvline(xaxisDatetime[13152], color='black', linestyle='--')
plt.xlim([xaxisDatetime[13000], xaxisDatetime[15000]])
plt.legend()
plt.ylim([1.2, 2.3])
...
ax1 = ax[0]
normFactor = np.nanstd(tmTruthPerturb)
ax1.bar(inds, [np.sqrt(np.nanmean(nrmseLSTMOnlinePerturb[startFrom:endAt])) / normFactor,
               np.sqrt(np.nanmean(nrmseLSTM1000Perturb[startFrom:endAt])) / normFactor,
               np.sqrt(np.nanmean(nrmseLSTM3000Perturb[startFrom:endAt])) / normFactor,
               np.sqrt(np.nanmean(nrmseLSTM6000Perturb[startFrom:endAt])) / normFactor,
               np.sqrt(np.nanmean(nrmseTMPerturb[startFrom:endAt])) / normFactor], width=width)
ax1.set_xticks(inds+width/2)
ax1.set_xticklabels( ('LSTMonline', 'LSTM1000', 'LSTM3000', 'LSTM6000', 'HTM') )
...
width = 0.5
inds = np.arange(6)
normFactor = np.nanmean(np.abs(tmTruthPerturb))
ax2.bar(inds, [np.nanmean(mapeELMPerturb[startFrom:endAt]) / normFactor,
               np.nanmean(mapeESNPerturb[startFrom:endAt]) / normFactor,
               np.nanmean(mapeLSTM1000Perturb[startFrom:endAt]) / normFactor,
               np.nanmean(mapeLSTM3000Perturb[startFrom:endAt]) / normFactor,
               np.nanmean(mapeLSTM6000Perturb[startFrom:endAt] / normFactor),
               np.nanmean(mapeTMPerturb[startFrom:endAt]) / normFactor], width=width)
ax2.set_xticks(inds+width/2)
ax2.set_xticklabels(('ELM', 'ESN', 'LSTM1000', 'LSTM3000',
                      'LSTM6000', 'HTM'))
...
ax3.bar(inds, [np.nanmean(negLLLSTMonlinePerturb[startFrom:endAt]),
               ...
               np.nanmean(negLLTMPerturb[startFrom:]), 0], width=width)
ax3.set_xticks(inds+width/2)
ax3.set_xticklabels( ('LSTMonline', 'LSTM1000', 'LSTM3000', 'LSTM6000', 'HTM','') )
